...
namespace TaskManager.Bll.Impl.Services.Task
{
    public class TaskService: ITaskService
    {
        ...
        {
            Entities.Tables.Task entityTask = await _unitOfWork.Tasks.GetByUnitIdAsync(taskId);
            if (entityTask == null)
            {
            ...
            model.Creator = _mapper.Map<UserModel>(
                (await _userService.GetUser(entityTask.Unit.CreatorId, entityTask.UnitId)).Data);
